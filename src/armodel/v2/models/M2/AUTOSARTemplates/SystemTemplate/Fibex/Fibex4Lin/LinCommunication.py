"""
AUTOSAR Package - LinCommunication
...
class ScheduleTableEntry(ARObject, ABC):
    ...
    def position_in_table(self, value: Optional["Integer"]) -> None:
        ...
        if not isinstance(value, (Integer, int)):
            raise TypeError(
                f"positionInTable must be Integer or int or None, got {type(value).__name__}"
            )
...
class FramePid(ARObject):
    ...
    def pid(self, value: Optional["PositiveInteger"]) -> None:
        ...
        if not isinstance(value, (PositiveInteger, str)):
            raise TypeError(
                f"pid must be PositiveInteger or str or None, got {type(value).__name__}"
            )
        self._pid = value
...
class AssignFrameIdRange(LinConfigurationEntry):
    ...
    def start_index(self, value: Optional["Integer"]) -> None:
        ...
        if not isinstance(value, (Integer, int)):
            raise TypeError(
                f"startIndex must be Integer or int or None, got {type(value).__name__}"
            )
...
class AssignNad(LinConfigurationEntry):
    ...
    def new_nad(self, value: Optional["Integer"]) -> None:
        ...
        if not isinstance(value, (Integer, int)):
            raise TypeError(
                f"newNad must be Integer or int or None, got {type(value).__name__}"
            )
        self._newNad = value
...
class LinChecksumType(AREnum):
    """
    LinChecksumType enumeration

Use of classic or enhanced checksum is managed by the master node and it is determined per frame identifier; Aggregated by LinFrameTriggering.linChecksum
...
The schedule table can be executed in two different modes. Aggregated by LinScheduleTable.runMode
...
class ResumePosition(AREnum):
    """
    ResumePosition enumeration

Defines, where a schedule table shall be proceeded in case if it has been interrupted by a run-once table or MRF/SRF. Aggregated by LinScheduleTable.resumePosition

Package: M2::AUTOSARTemplates::SystemTemplate::Fibex::Fibex4Lin::LinCommunication
    """
    # Continue at IT Point.
    continueAtItPosition = "0"
