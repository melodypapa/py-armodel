...
class TestSdgParsing:
    ...
    def test_readAdminDataSdgs(self, parser):
        element = _snip("<SDGS>" "<SDG GID='A'><SD>x</SD></SDG>" "<SDG GID='B'><SD>y</SD></SDG>" "</SDGS>")
        admin = AdminData()
...
class TestRevisionParsing:
    def test_readDocRevision_minimal(self, parser):
        element = _snip(
            "<DATE>2024-01-01T00:00:00</DATE>" "<ISSUED-BY>alice</ISSUED-BY>" "<REVISION-LABEL>1.0.0</REVISION-LABEL>" "<STATE>released</STATE>",
    ...
    def test_readDocRevision_with_modifications(self, parser):
        element = _snip(
            "<MODIFICATIONS>" "<MODIFICATION>" "<CHANGE><L-4>L-1</L-4></CHANGE>" "<REASON><L-4>R-1</L-4></REASON>" "</MODIFICATION>" "</MODIFICATIONS>",
            root_tag="DOC-REVISION",
        )
        revision = DocRevision()
        parser.readDocRevision(element, revision)
    ...
    def test_readAdminDataDocRevisions(self, parser):
        element = _snip("<DOC-REVISIONS>" "<DOC-REVISION>" "<ISSUED-BY>bob</ISSUED-BY>" "<REVISION-LABEL>2.0.0</REVISION-LABEL>" "</DOC-REVISION>" "</DOC-REVISIONS>")
        admin = AdminData()
...
class TestDocumentationBlockHandlers:
    ...
    def test_readDocumentationBlock_with_list(self, parser):
        element = _snip(
            "<LIST TYPE='number'>" "<ITEM><P><L-1 L='en'>item1</L-1></P></ITEM>" "</LIST>",
            root_tag="DOCUMENTATION-BLOCK",
        )
        block = DocumentationBlock()
        parser.readDocumentationBlock(element, block)
    ...
    def test_readDocumentationBlock_with_figure(self, parser):
        element = _snip(
            "<FIGURE>" "<L-GRAPHIC L='en'><GRAPHIC FILENAME='image.png'/></L-GRAPHIC>" "</FIGURE>",
            root_tag="DOCUMENTATION-BLOCK",
        )
        block = DocumentationBlock()
        parser.readDocumentationBlock(element, block)
    ...
    def test_getDocumentationBlockList(self, parser):
        element = _snip(
            "<ITEM><P><L-1 L='en'>item1</L-1></P></ITEM>" "<ITEM><P><L-1 L='en'>item2</L-1></P></ITEM>",
        )
    ...
    def test_getLParagraphs(self, parser):
        element = _snip(
            "<L-1 L='en'>para1</L-1>" "<L-1 L='de'>para2</L-1>",
        )
    ...
    def test_getLPlainTexts(self, parser):
        element = _snip(
            "<L-10 L='en'>plain1</L-10>" "<L-10 L='de'>plain2</L-10>",
        )
    ...
    def test_getMultiLanguagePlainText(self, parser):
        element = _snip(
            "<MY-TEXT>" "<L-10 L='en'>text1</L-10>" "<L-10 L='de'>text2</L-10>" "</MY-TEXT>",
        )
    ...
    def test_getListElements(self, parser):
        element = _snip(
            "<LIST TYPE='number'>" "<ITEM><P><L-1 L='en'>item</L-1></P></ITEM>" "</LIST>",
        )
...
class TestGraphicAndFigureHandlers:
    ...
    def test_readMlFigure_with_lgraphics(self, parser):
        element = _snip(
            "<L-GRAPHIC L='en'><GRAPHIC FILENAME='fig1.png'/></L-GRAPHIC>" "<L-GRAPHIC L='de'><GRAPHIC FILENAME='fig2.png'/></L-GRAPHIC>",
            root_tag="ML-FIGURE",
        )
        figure = MlFigure()
    ...
    def test_readMlFigureLGraphics(self, parser):
        element = _snip(
            "<L-GRAPHIC L='en'><GRAPHIC FILENAME='img.png'/></L-GRAPHIC>" "<L-GRAPHIC L='de'><GRAPHIC FILENAME='img2.png'/></L-GRAPHIC>",
            root_tag="FIGURE",
        )
        figure = MlFigure()
    ...
    def test_getMlFigures(self, parser):
        element = _snip(
            "<FIGURE>" "<L-GRAPHIC L='en'><GRAPHIC FILENAME='f1.png'/></L-GRAPHIC>" "</FIGURE>" "<FIGURE>" "<L-GRAPHIC L='de'><GRAPHIC FILENAME='f2.png'/></L-GRAPHIC>" "</FIGURE>",
        )
...
class TestAnnotationHandlers:
    ...
    def test_readGeneralAnnotation_with_text(self, parser):
        element = _snip(
            "<ANNOTATION-TEXT>" "<P><L-1 L='en'>note</L-1></P>" "</ANNOTATION-TEXT>",
            root_tag="ANNOTATION",
        )
        annotation = Annotation()
        parser.readGeneralAnnotation(element, annotation)
    ...
    def test_readGeneralAnnotation_with_label(self, parser):
        element = _snip(
            "<LABEL>" "<L-4 L='en'>LabelName</L-4>" "</LABEL>",
            root_tag="ANNOTATION",
        )
        annotation = Annotation()
        parser.readGeneralAnnotation(element, annotation)
    ...
    def test_getAnnotations_single(self, parser):
        element = _snip(
            "<ANNOTATIONS>" "<ANNOTATION><ANNOTATION-ORIGIN>tool</ANNOTATION-ORIGIN></ANNOTATION>" "</ANNOTATIONS>",
        )
    ...
    def test_getAnnotations_multiple(self, parser):
        element = _snip(
            "<ANNOTATIONS>" "<ANNOTATION><ANNOTATION-ORIGIN>tool1</ANNOTATION-ORIGIN></ANNOTATION>" "<ANNOTATION><ANNOTATION-ORIGIN>tool2</ANNOTATION-ORIGIN></ANNOTATION>" "</ANNOTATIONS>",
        )
        annotations = parser.getAnnotations(element)
...
class TestSwDataDefPropsHandlers:
    ...
    def test_getSwPointerTargetProps(self, parser):
        element = _snip(
            "<SW-POINTER-TARGET-PROPS>" "<TARGET-CATEGORY>variable</TARGET-CATEGORY>" "</SW-POINTER-TARGET-PROPS>",
        )
    ...
    def test_getSwAxisIndividual(self, parser):
        element = _snip(
            "<SW-MAX-AXIS-POINTS>100</SW-MAX-AXIS-POINTS>" "<SW-MIN-AXIS-POINTS>2</SW-MIN-AXIS-POINTS>",
...
class TestSdgDeepHandlers:
    ...
    def test_readAdminDataSdgs_nested_sdg(self, parser):
        element = _snip(
            "<SDGS>" "<SDG GID='Parent'>" "<SDG GID='Child'><SD>childValue</SD></SDG>" "</SDG>" "</SDGS>",
        )
        admin = AdminData()
        parser.readAdminDataSdgs(element, admin)
        sdgs = admin.getSdgs()
...
class TestDocRevisionModificationGap:
    def test_unsupported_modification_tag_warns(self, warning_parser, caplog):
        ...
        revision = DocRevision()
        element = _snip("<MODIFICATIONS><UNKNOWN-MOD>" "<SHORT-NAME>m</SHORT-NAME>" "</UNKNOWN-MOD></MODIFICATIONS>")
        with caplog.at_level(logging.ERROR):
            warning_parser.readDocRevisionModifications(element, revision)
        assert any("Unsupported Modification" in r.getMessage() for r in caplog.records)
