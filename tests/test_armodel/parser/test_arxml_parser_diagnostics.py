...
class TestDoIpHandlers:
    ...
    def test_readDoIpLogicAddress_sets_address(self, parser):
        ...
        element = _snip(
            "<SHORT-NAME>doIpAddr</SHORT-NAME>" "<ADDRESS>0x0E80</ADDRESS>",
    ...
    def test_readDoIpTpConfigDoIpLogicAddresses_with_address(self, parser):
        ...
        element = _snip(
            "<DO-IP-LOGIC-ADDRESSS>" "<DO-IP-LOGIC-ADDRESS>" "<SHORT-NAME>logicAddr</SHORT-NAME>" "<ADDRESS>0x0E80</ADDRESS>" "</DO-IP-LOGIC-ADDRESS>" "</DO-IP-LOGIC-ADDRESSS>",
        )
    ...
    def test_readDoIpTpConfigDoIpLogicAddresses_unknown_warning(self, warning_parser):
        ...
        config = DoIpTpConfig(parent=_autosar_root(), short_name="doIpConfig")
        element = _snip(
            "<DO-IP-LOGIC-ADDRESSS>" "<UNKNOWN-ADDRESS>" "<SHORT-NAME>Unknown</SHORT-NAME>" "</UNKNOWN-ADDRESS>" "</DO-IP-LOGIC-ADDRESSS>",
        )
    ...
    def test_readDoIpTpConfigTpConnections_unknown_warning(self, warning_parser):
        ...
        config = DoIpTpConfig(parent=_autosar_root(), short_name="doIpConfig")
        element = _snip(
            "<TP-CONNECTIONS>" "<UNKNOWN-CONNECTION>" "<SHORT-NAME>Unknown</SHORT-NAME>" "</UNKNOWN-CONNECTION>" "</TP-CONNECTIONS>",
        )
...
class TestServiceInstanceGaps:
    def test_readConsumedServiceInstance_sets_refs(self, parser):
        ...
        end_point = ApplicationEndpoint(parent=MagicMock(), short_name="Aep")
        instance = end_point.createConsumedServiceInstance("Csi")
        element = _snip('<PROVIDED-SERVICE-INSTANCE-REF DEST="PROVIDED-SERVICE-INSTANCE">/psi</PROVIDED-SERVICE-INSTANCE-REF>' "<SD-CLIENT-CONFIG/>")
        parser.readConsumedServiceInstance(element, instance)
        assert instance.getProvidedServiceInstanceRef() is not None

    def test_readSocketAddressApplicationEndpointConsumedServiceInstances_unsupported_warns(self, warning_parser, caplog):
        from armodel.models import ApplicationEndpoint

        end_point = ApplicationEndpoint(parent=MagicMock(), short_name="Aep")
        element = _snip("<CONSUMED-SERVICE-INSTANCES><BAD/></CONSUMED-SERVICE-INSTANCES>")
        with caplog.at_level(logging.ERROR):
            warning_parser.readSocketAddressApplicationEndpointConsumedServiceInstances(element, end_point)
        assert any("Unsupported ConsumedServiceInstances" in r.getMessage() for r in caplog.records)

    def test_readSocketAddressApplicationEndpointConsumedServiceInstances_creates(self, parser):
        from armodel.models import ApplicationEndpoint

        end_point = ApplicationEndpoint(parent=MagicMock(), short_name="Aep")
        element = _snip("<CONSUMED-SERVICE-INSTANCES>" "<CONSUMED-SERVICE-INSTANCE>" "<SHORT-NAME>csi</SHORT-NAME>" "</CONSUMED-SERVICE-INSTANCE>" "</CONSUMED-SERVICE-INSTANCES>")
        parser.readSocketAddressApplicationEndpointConsumedServiceInstances(element, end_point)
        assert len(end_point.getConsumedServiceInstances()) == 1

    def test_readSocketAddressApplicationEndpointProvidedServiceInstances_creates(self, parser):
        from armodel.models import ApplicationEndpoint

        end_point = ApplicationEndpoint(parent=MagicMock(), short_name="Aep")
        element = _snip("<PROVIDED-SERVICE-INSTANCES>" "<PROVIDED-SERVICE-INSTANCE>" "<SHORT-NAME>psi</SHORT-NAME>" "</PROVIDED-SERVICE-INSTANCE>" "</PROVIDED-SERVICE-INSTANCES>")
        parser.readSocketAddressApplicationEndpointProvidedServiceInstance(element, end_point)
        assert len(end_point.getProvidedServiceInstances()) == 1

    def test_readEventHandler_adds_consumed_event_group_ref(self, parser):
        from armodel.models import ProvidedServiceInstance

        instance = ProvidedServiceInstance(parent=MagicMock(), short_name="Psi")
    ...
    def test_readProvidedServiceInstanceEventHandlers_unsupported_warns(self, warning_parser, caplog):
        from armodel.models import ProvidedServiceInstance

        instance = ProvidedServiceInstance(parent=MagicMock(), short_name="Psi")
        element = _snip("<EVENT-HANDLERS><BAD/></EVENT-HANDLERS>")
        with caplog.at_level(logging.ERROR):
            warning_parser.readProvidedServiceInstanceEventHandlers(element, instance)
        assert any("Unsupported Event Handler" in r.getMessage() for r in caplog.records)

    def test_readSocketAddressApplicationEndpointProvidedServiceInstance_unsupported_warns(self, warning_parser, caplog):
        from armodel.models import ApplicationEndpoint

        end_point = ApplicationEndpoint(parent=MagicMock(), short_name="Aep")
        element = _snip("<PROVIDED-SERVICE-INSTANCES><BAD/></PROVIDED-SERVICE-INSTANCES>")
        with caplog.at_level(logging.ERROR):
            warning_parser.readSocketAddressApplicationEndpointProvidedServiceInstance(element, end_point)
        assert any("Unsupported ConsumedServiceInstances" in r.getMessage() for r in caplog.records)
...
class TestDiagnosticConnectionGaps:
    def test_readDiagnosticConnectionFunctionalRequestRefs_adds_ref(self, parser):
        from armodel.models import DiagnosticConnection

        conn = DiagnosticConnection(parent=MagicMock(), short_name="Dc")
        element = _snip("<FUNCTIONAL-REQUEST-REFS>" '<FUNCTIONAL-REQUEST-REF DEST="DIAGNOSTIC-CONNECTION">/fr</FUNCTIONAL-REQUEST-REF>' "</FUNCTIONAL-REQUEST-REFS>")
    ...
    def test_readDiagnosticServiceTableDiagnosticConnectionRefs_adds(self, parser):
        ...
        table = DiagnosticServiceTable(parent=MagicMock(), short_name="Dst")
        element = _snip(
        ...
        parser.readDiagnosticServiceTableDiagnosticConnectionRefs(element, table)
...
class TestDiagEventDebounceAlgorithm:
    def test_readDiagEventDebounceAlgorithm_creates_monitor_internal(self, parser):
        from armodel.models import DiagnosticEventNeeds

        needs = DiagnosticEventNeeds(parent=MagicMock(), short_name="Den")
        element = _snip(
            "<DIAG-EVENT-DEBOUNCE-ALGORITHM>" "<DIAG-EVENT-DEBOUNCE-MONITOR-INTERNAL>" "<SHORT-NAME>Di</SHORT-NAME>" "</DIAG-EVENT-DEBOUNCE-MONITOR-INTERNAL>" "</DIAG-EVENT-DEBOUNCE-ALGORITHM>"
        )
    ...
    def test_readDiagEventDebounceAlgorithm_unsupported_warns(self, warning_parser, caplog):
        ...
        needs = DiagnosticEventNeeds(parent=MagicMock(), short_name="Den")
        element = _snip("<DIAG-EVENT-DEBOUNCE-ALGORITHM><BAD/></DIAG-EVENT-DEBOUNCE-ALGORITHM>")
        with caplog.at_level(logging.ERROR):
