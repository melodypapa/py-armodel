...
class TestTDEventTTCanCycleStart:
    def _parent(self):
    ...
    def test_base_is_tdevent_cycle_start(self):
        assert issubclass(TDEventTTCanCycleStart, TDEventCycleStart)

    def test_defaults(self):
        parent = self._parent()
        event = TDEventTTCanCycleStart(parent, "E1")
        assert event.getTtCanClusterRef() is None
        assert event.getCycleRepetition() is None
        assert event.getEcuInstanceRef() is None

    def test_set_get_tt_can_cluster_ref(self):
        parent = self._parent()
        event = TDEventTTCanCycleStart(parent, "E1")
        ref = RefType()
        ref.setValue("/Path/To/TtcanCluster")
        assert event.setTtCanClusterRef(ref) is event
        assert event.getTtCanClusterRef() is ref

    def test_set_tt_can_cluster_ref_none_noop(self):
        parent = self._parent()
        event = TDEventTTCanCycleStart(parent, "E1")
        ref = RefType()
        ref.setValue("/Path/To/TtcanCluster")
        event.setTtCanClusterRef(ref)
        event.setTtCanClusterRef(None)
        assert event.getTtCanClusterRef() is ref
