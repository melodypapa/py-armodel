...
from armodel.models.M2.AUTOSARTemplates.GenericStructure.GeneralTemplateClasses.PrimitiveTypes import Boolean, Limit, RefType, TimeValue
...
ECU_INSTANCE_NOTES = {
    "associatedComIPduGroup": (
        ...
        "Only top level ISignalIPduGroups shall be referenced by an EcuInstance. "
        "If an ISignalIPduGroup contains other ISignalIPduGroups than these contained ISignalIPduGroups shall not be referenced by the EcuInstance. "
        "Contained ISignalIPduGroups are associated to an EcuInstance via the top level ISignalIPduGroup."
    ),
    "associatedConsumedProvidedServiceInstanceGroup": ("With this reference it is possible to identify which ConsumedProvidedServiceInstanceGroups are applicable for which ECUInstance."),
    "associatedPdurIPduGroup": ("With this reference it is possible to identify which PduR IPdu Groups are applicable for which CommunicationConnector/ ECU."),
    "channelSynchronousWakeup": (
        "If this parameter is available and set to true, then all available channels will be woken up as soon as at least one "
        "channel wakeup occurs. If PNCs are configured, then all PNCs will be requested upon a channel wakeup."
    ),
    "clientIdRange": (
        "Restriction of the Client Identifier for this Ecu to an allowed range of numerical values. "
        "The Client Identifier of the transaction handle is generated by the client RTE for inter-Ecu Client/Server communication."
    ),
    "comConfigurationGwTimeBase": "The period between successive calls to Com_MainFunctionRouteSignals of the AUTOSAR COM module in seconds.",
    "comConfigurationRxTimeBase": "The period between successive calls to Com_MainFunctionRx of the AUTOSAR COM module in seconds.",
    "comConfigurationTxTimeBase": "The period between successive calls to Com_MainFunctionTx of the AUTOSAR COM module in seconds.",
    ...
    "commController": "CommunicationControllers of the ECU.",
    "connector": "All channels controlled by a single controller.",
    "dltConfig": "Describes the Dlt configuration on this EcuInstance.",
    "doIpConfig": "DoIp configuration on this EcuInstance.",
    "ecuTaskProxy": "Reference to OsTaskProxies assigned to the EcuInstance.",
    ...
    "firewallRule": "Firewall rules defined in the context of an EcuInstance.",
    "partition": "Optional definition of Partitions within an Ecu.",
    "pncNmRequest": ("Defines if this EcuInstance shall request Nm on all its PhysicalChannels which have Nm variant set to FULL " "each time a PNC is requested."),
    "pncPrepareSleepTimer": "Time in seconds the PNC state machine shall wait in PNC_PREPARE_SLEEP.",
    ...
    "pnResetTime": ("Specifies the runtime of the reset timer in seconds. " "This reset time is valid for the reset of PN requests in the EIRA and in the ERA."),
    "sleepModeSupported": (
    ...
    "tcpIpIcmpProps": "EcuInstance specific ICMP (Internet Control Message Protocol) attributes",
    "tcpIpProps": "EcuInstance specific TcpIp Stack attributes.",
    "v2xSupported": "This attribute is used to control the existence of the V2X stack on the given EcuInstance.",
    "wakeUpOverBusSupported": "Driver support for wakeup over Bus.",
}
...
class Test_FibexCoreEcuInstance:
    ...
    def test_EcuInstance_spec_docstrings(self):
        """Test Table 3.1 docstrings: class Note verbatim, docless __init__, per-attribute Notes verbatim."""
        paragraphs = EcuInstance.__doc__.strip().split("\n\n")
        assert paragraphs[0] == ECU_INSTANCE_CLASS_NOTE
        assert paragraphs[1].strip() == ECU_INSTANCE_CONSTR_3008
        assert EcuInstance.__init__.__doc__ is None

        accessor_notes = [
            ("associatedComIPduGroup", "addAssociatedComIPduGroupRef", "getAssociatedComIPduGroupRefs"),
            ("associatedConsumedProvidedServiceInstanceGroup", "addAssociatedConsumedProvidedServiceInstanceGroupRef", "getAssociatedConsumedProvidedServiceInstanceGroupRefs"),
            ("associatedPdurIPduGroup", "addAssociatedPdurIPduGroupRef", "getAssociatedPdurIPduGroupRefs"),
            ("channelSynchronousWakeup", "setChannelSynchronousWakeup", "getChannelSynchronousWakeup"),
            ("clientIdRange", "setClientIdRange", "getClientIdRange"),
            ("comConfigurationGwTimeBase", "setComConfigurationGwTimeBase", "getComConfigurationGwTimeBase"),
            ("comConfigurationRxTimeBase", "setComConfigurationRxTimeBase", "getComConfigurationRxTimeBase"),
            ("comConfigurationTxTimeBase", "setComConfigurationTxTimeBase", "getComConfigurationTxTimeBase"),
            ("comEnableMDTForCyclicTransmission", "setComEnableMDTForCyclicTransmission", "getComEnableMDTForCyclicTransmission"),
            ("commController", "createCanCommunicationController", "getCommControllers"),
            ("connector", "createCanCommunicationConnector", "getConnectors"),
            ("dltConfig", "setDltConfig", "getDltConfig"),
            ("doIpConfig", "setDoIpConfig", "getDoIpConfig"),
            ("ecuTaskProxy", "addEcuTaskProxyRef", "getEcuTaskProxyRefs"),
            ("ethSwitchPortGroupDerivation", "setEthSwitchPortGroupDerivation", "getEthSwitchPortGroupDerivation"),
            ("firewallRule", "addFirewallRuleRef", "getFirewallRuleRefs"),
            ("partition", "addPartition", "getPartitions"),
            ("pncNmRequest", "setPncNmRequest", "getPncNmRequest"),
            ("pncPrepareSleepTimer", "setPncPrepareSleepTimer", "getPncPrepareSleepTimer"),
            ("pncSynchronousWakeup", "setPncSynchronousWakeup", "getPncSynchronousWakeup"),
            ("pnResetTime", "setPnResetTime", "getPnResetTime"),
            ("sleepModeSupported", "setSleepModeSupported", "getSleepModeSupported"),
            ("tcpIpIcmpProps", "setTcpIpIcmpPropsRef", "getTcpIpIcmpPropsRef"),
            ("tcpIpProps", "setTcpIpPropsRef", "getTcpIpPropsRef"),
            ("v2xSupported", "setV2xSupported", "getV2xSupported"),
            ("wakeUpOverBusSupported", "setWakeUpOverBusSupported", "getWakeUpOverBusSupported"),
        ]
        for attr, mutator_name, getter_name in accessor_notes:
            note = ECU_INSTANCE_NOTES[attr]
            getter = getattr(EcuInstance, getter_name)
            mutator = getattr(EcuInstance, mutator_name)
            assert getter.__doc__ is not None and getter.__doc__.strip() == note, getter_name
            assert mutator.__doc__ is not None and mutator.__doc__.strip().startswith(note), mutator_name

    def test_EcuInstance_spec_optional_typing(self):
        """Test Table 3.1 typing contract: 0..1 -> Optional[T], * -> List[T] (Rule 0001.4)."""
        hints = typing.get_type_hints(EcuInstance.getAssociatedComIPduGroupRefs)
        assert hints["return"] == typing.List[RefType]
        hints = typing.get_type_hints(EcuInstance.addAssociatedComIPduGroupRef)
        assert hints["value"] == typing.Optional[RefType]

        hints = typing.get_type_hints(EcuInstance.getEcuTaskProxyRefs)
        assert hints["return"] == typing.List[RefType]
        hints = typing.get_type_hints(EcuInstance.addEcuTaskProxyRef)
        assert hints["value"] == typing.Optional[RefType]

        hints = typing.get_type_hints(EcuInstance.getFirewallRuleRefs)
        assert hints["return"] == typing.List[RefType]
        hints = typing.get_type_hints(EcuInstance.addFirewallRuleRef)
        assert hints["value"] == typing.Optional[RefType]

        hints = typing.get_type_hints(EcuInstance.getCommControllers)
        assert hints["return"] == typing.List[CommunicationController]
        hints = typing.get_type_hints(EcuInstance.getConnectors)
        assert hints["return"] == typing.List[CommunicationConnector]

        for getter, setter, value_type in [
            ("getChannelSynchronousWakeup", "setChannelSynchronousWakeup", Boolean),
            ("getComConfigurationGwTimeBase", "setComConfigurationGwTimeBase", TimeValue),
            ("getComConfigurationRxTimeBase", "setComConfigurationRxTimeBase", TimeValue),
            ("getComConfigurationTxTimeBase", "setComConfigurationTxTimeBase", TimeValue),
            ("getComEnableMDTForCyclicTransmission", "setComEnableMDTForCyclicTransmission", Boolean),
            ("getEthSwitchPortGroupDerivation", "setEthSwitchPortGroupDerivation", Boolean),
            ("getPncNmRequest", "setPncNmRequest", Boolean),
            ("getPncPrepareSleepTimer", "setPncPrepareSleepTimer", TimeValue),
            ("getPncSynchronousWakeup", "setPncSynchronousWakeup", Boolean),
            ("getPnResetTime", "setPnResetTime", TimeValue),
            ("getSleepModeSupported", "setSleepModeSupported", Boolean),
            ("getTcpIpIcmpPropsRef", "setTcpIpIcmpPropsRef", RefType),
            ("getTcpIpPropsRef", "setTcpIpPropsRef", RefType),
            ("getWakeUpOverBusSupported", "setWakeUpOverBusSupported", Boolean),
        ]:
            hints = typing.get_type_hints(getattr(EcuInstance, getter))
            assert hints["return"] == typing.Optional[value_type], getter
            hints = typing.get_type_hints(getattr(EcuInstance, setter))
            assert hints["value"] == typing.Optional[value_type], setter

        # placeholder-typed members: forward references to classes not yet implemented (Rule 0001.10);
        # the module uses `from __future__ import annotations`, so raw annotation strings are compared
        # (quote characters stripped - quote style differs across Python versions)
        def raw_annotation(func, key):
            return func.__annotations__[key].replace("'", "").replace('"', "")

        assert raw_annotation(EcuInstance.getClientIdRange, "return") == "Optional[ClientIdRange]"
        assert raw_annotation(EcuInstance.setClientIdRange, "value") == "Optional[ClientIdRange]"
        assert raw_annotation(EcuInstance.getDltConfig, "return") == "Optional[DltConfig]"
        assert raw_annotation(EcuInstance.setDltConfig, "value") == "Optional[DltConfig]"
        assert raw_annotation(EcuInstance.getDoIpConfig, "return") == "Optional[DoIpConfig]"
        assert raw_annotation(EcuInstance.setDoIpConfig, "value") == "Optional[DoIpConfig]"
        assert raw_annotation(EcuInstance.getPartitions, "return") == "List[EcuPartition]"
        assert raw_annotation(EcuInstance.addPartition, "value") == "Optional[EcuPartition]"
        assert raw_annotation(EcuInstance.getV2xSupported, "return") == "Optional[V2xSupportEnum]"
        assert raw_annotation(EcuInstance.setV2xSupported, "value") == "Optional[V2xSupportEnum]"

    def test_EcuInstance_none_noop_guards(self):
        """Test Table 3.1 scalar setters and list adders are no-ops on None (Rule 0004)."""
        parent = MockParent()
        ecu = EcuInstance(parent, "test_ecu_none_noop")

        boolean_value = Boolean()
        boolean_value.setValue(True)
        assert ecu == ecu.setChannelSynchronousWakeup(boolean_value)
        assert ecu == ecu.setChannelSynchronousWakeup(None)
        assert ecu.getChannelSynchronousWakeup() == boolean_value

        gw_time = TimeValue()
        gw_time.setValue("0.02")
        assert ecu == ecu.setComConfigurationGwTimeBase(gw_time)
        assert ecu == ecu.setComConfigurationGwTimeBase(None)
        assert ecu.getComConfigurationGwTimeBase() == gw_time

        icmp_ref = RefType()
        icmp_ref.setValue("/EthSystem/ECUS/Ecu1/TCP-IP-ICMP-PROPS")
        assert ecu == ecu.setTcpIpIcmpPropsRef(icmp_ref)
        assert ecu == ecu.setTcpIpIcmpPropsRef(None)
        assert ecu.getTcpIpIcmpPropsRef() == icmp_ref

        ref = RefType()
        ref.setValue("/Systems/S/ISIGNAL-I-PDU-GROUPS/G1")
        ecu.addAssociatedComIPduGroupRef(ref)
        assert ecu == ecu.addAssociatedComIPduGroupRef(None)
        assert ecu.getAssociatedComIPduGroupRefs() == [ref]

        ecu.addEcuTaskProxyRef(ref)
        assert ecu == ecu.addEcuTaskProxyRef(None)
        assert ecu.getEcuTaskProxyRefs() == [ref]

        ecu.addFirewallRuleRef(ref)
        assert ecu == ecu.addFirewallRuleRef(None)
        assert ecu.getFirewallRuleRefs() == [ref]

    def test_EcuInstance_dedicated_list_fields(self):
        """Test commController/connector getters read the dedicated typed list fields in insertion order (Rule 0004)."""
        parent = MockParent()
        ecu = EcuInstance(parent, "test_ecu_fields")

        second = ecu.createCanCommunicationController("b_controller")
        first = ecu.createEthernetCommunicationController("a_controller")
        assert ecu.getCommControllers() == [second, first]  # insertion order, not registry-sorted

        connector_second = ecu.createCanCommunicationConnector("b_connector")
        connector_first = ecu.createEthernetCommunicationConnector("a_connector")
        assert ecu.getConnectors() == [connector_second, connector_first]

        # The concrete classes are TYPE_CHECKING-only imports in CoreTopology - resolve via localns
        localns = {
            "CanCommunicationController": CanCommunicationController,
            "CanCommunicationConnector": CanCommunicationConnector,
        }
        hints = typing.get_type_hints(EcuInstance.createCanCommunicationController, localns=localns)
