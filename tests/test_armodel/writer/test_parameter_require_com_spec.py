"""
Tests for writing PARAMETER-REQUIRE-COM-SPEC elements — ParameterRequireComSpec, Table 4.83 (p.193, R23-11).
...
and parameter (PARAMETER-REF 0..1 reference to a ParameterDataPrototype). Writer
element order must follow the XSD sequenceOffset (AUTOSAR_00052.xsd group
PARAMETER-REQUIRE-COM-SPEC: INIT-VALUE → PARAMETER-REF).
...
Round-trip counterpart: tests/test_armodel/parser/test_parameter_require_com_spec.py
"""
...
class TestWriteParameterRequireComSpec:
    """Tests for writeParameterRequireComSpec — own element field values (Table 4.83)."""
    ...
    def test_write_field_values(self, writer):
        ...
        com_spec.setParameterRef(_ref("/pkg/ParamInterface/Param", "PARAMETER-DATA-PROTOTYPE"))
        parent = ET.Element("PARENT")

        writer.writeParameterRequireComSpec(parent, com_spec)

        child = parent.find("PARAMETER-REQUIRE-COM-SPEC")
        assert child is not None
        assert child.find("INIT-VALUE") is not None
        assert child.find("INIT-VALUE/TEXT-VALUE-SPECIFICATION/VALUE").text == "0"
        assert child.find("PARAMETER-REF").text == "/pkg/ParamInterface/Param"
        assert child.find("PARAMETER-REF").attrib.get("DEST") == "PARAMETER-DATA-PROTOTYPE"
    ...
    def test_write_xsd_element_order(self, writer):
        ...
        child = parent.find("PARAMETER-REQUIRE-COM-SPEC")
        assert [elem.tag for elem in child] == ["INIT-VALUE", "PARAMETER-REF"]
...
class TestParameterRequireComSpecRoundTrip:
    ...
    def test_round_trip_field_values(self, writer):
        ...
        parent = ET.Element("PARENT")
        writer.writeParameterRequireComSpec(parent, com_spec)
        element = parent.find("PARAMETER-REQUIRE-COM-SPEC")

        xml_text = ET.tostring(element, encoding="unicode")
        reloaded_element = ET.fromstring(xml_text.replace("PARAMETER-REQUIRE-COM-SPEC", "PARAMETER-REQUIRE-COM-SPEC xmlns='http://autosar.org/schema/r4.0'", 1))
        reloaded = ARXMLParser().getParameterRequireComSpec(reloaded_element)
        assert reloaded is not None
        assert isinstance(reloaded.getInitValue(), TextValueSpecification)
        assert reloaded.getInitValue().getValue().getValue() == "0"
        assert reloaded.getParameterRef() is not None
        assert reloaded.getParameterRef().getValue() == "/pkg/ParamInterface/Param"
        assert reloaded.getParameterRef().getDest() == "PARAMETER-DATA-PROTOTYPE"
    ...
    def test_round_trip_via_required_com_spec_dispatch(self, writer):
        ...
        assert wrapper.find("PARAMETER-REQUIRE-COM-SPEC") is not None
        ...
        assert len(specs) == 1
        reloaded = specs[0]
        assert isinstance(reloaded, ParameterRequireComSpec)
        assert isinstance(reloaded.getInitValue(), TextValueSpecification)
        assert reloaded.getInitValue().getValue().getValue() == "0"
        assert reloaded.getParameterRef() is not None
        assert reloaded.getParameterRef().getValue() == "/pkg/ParamInterface/Param"
