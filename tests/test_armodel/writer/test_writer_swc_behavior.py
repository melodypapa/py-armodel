...
class TestWriterRteEvents:
    ...
    def test_writeDataReceivedEvent_optional_children_omitted(self, writer):
        behavior = _make_behavior()
        event = behavior.createDataReceivedEvent("dre")
        parent = _parent()
        writer.writeDataReceivedEvent(parent, event)
    ...
    def test_writeDataSendCompletedEvent_optional_children_omitted(self, writer):
        behavior = _make_behavior()
        event = behavior.createDataSendCompletedEvent("dsc")
        parent = _parent()
        writer.writeDataSendCompletedEvent(parent, event)
...
class TestDataReceivedEventRoundTrip:
    def test_round_trip_populated(self):
        """Test set -> save -> reload of a DataReceivedEvent with dataIRef."""
        import os
        import tempfile

        from armodel.models.M2.AUTOSARTemplates.SWComponentTemplate.SwcInternalBehavior.RTEEvents import DataReceivedEvent
        from armodel.parser.arxml_parser import ARXMLParser
        ...
        app = document.createARPackage("Pkg").createApplicationSwComponentType("App")
        behavior = app.createSwcInternalBehavior("Behavior")
        event = behavior.createDataReceivedEvent("dre1")
        iref = RVariableInAtomicSwcInstanceRef()
        iref.setContextRPortRef(_ref("/Pkg/App/rp1", "R-PORT-PROTOTYPE"))
        iref.setTargetDataElementRef(_ref("/Pkg/App/de1", "VARIABLE-DATA-PROTOTYPE"))
        event.setDataIRef(iref)
    ...
    def test_round_trip_empty(self):
        """Test that a DataReceivedEvent without dataIRef round-trips without the element."""
        import os
        import tempfile
        import xml.etree.ElementTree as ET
        ...
        app = document.createARPackage("Pkg").createApplicationSwComponentType("App")
        behavior = app.createSwcInternalBehavior("Behavior")
        behavior.createDataReceivedEvent("dre1")
        ...
        try:
            ...
            with open(file_path, "r", encoding="utf-8") as f:
                saved = ET.parse(f).getroot()
            evt = next(e for e in saved.iter() if e.tag.endswith("DATA-RECEIVED-EVENT"))
            assert all(not c.tag.endswith("DATA-IREF") for c in evt)
            document.clear()
            ARXMLParser().load(file_path, document)
            package = document.getARPackages()[0]
...
class TestDataSendCompletedEventRoundTrip:
    def test_round_trip_populated(self):
        """Test set -> save -> reload of a DataSendCompletedEvent with eventSourceRef."""
        import os
        import tempfile
        ...
        app = document.createARPackage("Pkg").createApplicationSwComponentType("App")
        behavior = app.createSwcInternalBehavior("Behavior")
        event = behavior.createDataSendCompletedEvent("dsc1")
    ...
    def test_round_trip_empty(self):
        """Test that a DataSendCompletedEvent without eventSourceRef round-trips without the element."""
        import os
        import tempfile
        import xml.etree.ElementTree as ET
        ...
        app = document.createARPackage("Pkg").createApplicationSwComponentType("App")
        behavior = app.createSwcInternalBehavior("Behavior")
        behavior.createDataSendCompletedEvent("dsc1")
        ...
        try:
            ...
            with open(file_path, "r", encoding="utf-8") as f:
                saved = ET.parse(f).getroot()
            evt = next(e for e in saved.iter() if e.tag.endswith("DATA-SEND-COMPLETED-EVENT"))
            assert all(not c.tag.endswith("EVENT-SOURCE-REF") for c in evt)
            document.clear()
            ARXMLParser().load(file_path, document)
            package = document.getARPackages()[0]
