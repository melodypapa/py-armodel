...
def run_pytest_with_coverage(include_slow_tests=True, test_path="tests/test_armodel", exclude_patterns=None):
    ...
    try:
        ...
        cmd = [sys.executable, "-m", "pytest", test_path, "--cov=src/armodel", "--cov-report=term-missing", "--tb=short", "--maxfail=1"]  # Stop after first failure to prevent long execution
...
def main():
    ...
    parser.add_argument("--skip-slow-tests", action="store_true", help="Skip slow tests to reduce execution time")
    parser.add_argument("--test-path", default="tests/test_armodel", help="Specify which tests to run (default: tests/test_armodel)")
    parser.add_argument("--exclude", action="append", default=[], help="Patterns to exclude from testing (can be used multiple times)")
    ...
    coverage_output = run_pytest_with_coverage(include_slow_tests=not args.skip_slow_tests, test_path=args.test_path, exclude_patterns=exclude_patterns)
