...
- The pattern `if short_name not in self.elements` was used in **56 `create<Name>()` factory methods across 13 model files**. `self.elements` is a list of *objects*, so the string-membership test was always true. `addElement`'s internal guard prevented duplicate mappings, but side-lists (e.g. `networkEndpoints`) still received duplicates on repeated create calls (double emission by the writer).
- Additionally, **166 pre-existing type-less guards** `IsElementExists(short_name)` (in `create*`/`_create*` factory methods) were also qualified with the constructed type — a type-less check wrongly *blocks* the legal AUTOSAR case of same short name / different type (per §2.3).
- **Fix applied (three parts):**
  ...
  3. Remove the redundant `if hasattr(new_element, "getShortName"): self.addElement(...)` wrapper (every constructed element is a `Referrable` subclass, so the check was always true) — `addElement` is called directly.
- One of the 56 broken guards lived in a misnamed factory: `NmCluster.readUdpNmNode` (a create-style factory with a `read` prefix; see `arxml_parser_code_review.md` §10). Its guard was fixed; the method name is kept to avoid touching callers — renaming is tracked separately.
