...
## `BswInterruptEntity`
- **PDF:** `AUTOSAR_CP_TPS_BSWModuleDescriptionTemplate.pdf`  | **page:** 75  | **table:** Table 5.8
- **Package:** `M2::AUTOSARTemplates::BswModuleTemplate::BswBehavior`
- **Source:** `src/armodel/models/M2/AUTOSARTemplates/BswModuleTemplate/BswBehavior.py`

| Name in source code | Type (source) | Member name (spec) | Type (PDF) | Kind | Deviation |
|---|---|---|---|---|---|
| — *(no deviations)* | — | — | — | - | none (2026-09-26 sync: R23-11 Table 5.8; orphan-intake drift fixed — fabricated class docstring replaced with the Note verbatim, `__init__` docstring removed, bare `BswInterruptCategory = None` / `String = None` fields retyped `Optional[BswInterruptCategory]` / `Optional[String]`, accessors typed, None no-op added, old 4-col fake intake checklist removed and rebuilt 6-col (no pre-existing `# Spec verified:`/`# XSD verified:` marker found); attrs `interruptCategory` (BswInterruptCategory, 0..1, XSD element `INTERRUPT-CATEGORY`, tokens `CAT-1`/`CAT-2` via new `BSW_INTERRUPT_CATEGORY_XML_MAP`) + `interruptSource` (String, 0..1, XSD element `INTERRUPT-SOURCE`, typed `getChildElementOptionalString`/`setChildElementOptionalString`); Base most-derived `BswModuleEntity` (already correct); VP capability inherited from base — no VARIATION-POINT element in XSD group `BSW-INTERRUPT-ENTITY` (AUTOSAR_00052.xsd L10907), arrives via the base group ref L10941 (Rule 0020); reader drift FIXED — `readBswInterruptEntity` produced generic `ARLiteral` for both attrs, now maps the enum token to `BswInterruptCategory` and reads `String`; writer drift FIXED — `setBswInterruptEntity` renamed `writeBswInterruptEntity` (matched pair with the reader, siblings `writeBswCalledEntity`/`writeBswSchedulableEntity`), enum written via the token map, source via the typed helper. Member type `BswInterruptCategory` exists with correct Table 5.9 literals (`cat1`/`cat2`) — its own stale checklist/docstrings are deferred to its own pass. Stamp deferred to batch confirmation.) |
...
Stale row removed 2026-09-18: `associatedcomipdugrouprefs` / `type (spec many vs py single)` — the field is the spec-many list `associatedComIPduGroupRefs: List[RefType]`. Placeholder resolved 2026-09-20: `partition` / `EcuPartition` (Table 5.7) implemented — factory `createEcuPartition(short_name)` per the spec shape, reader `readEcuInstancePartitions` (`PARTITIONS`/`ECU-PARTITION` wrapper per AUTOSAR_00052.xsd group line 50252) + writer `writeEcuInstancePartitions` wired; `addPartition` kept as an added convenience mutator (no reader/writer route; row marked `[—]`/`[—]`). No other deviations; all 26 Table 3.1 attributes are modeled (assoc. refs via `RefType` lists, controllers/connectors via dedicated typed list fields + per-subtype factories), reader/writer cover every attribute whose child classes exist, wrapper `ASSOCIATED-CONSUMED-PROVIDED-SERVICE-INSTANCE-GROUPS` + `CONSUMED-PROVIDED-SERVICE-INSTANCE-GROUP-REF-CONDITIONAL` and refs `TCP-IP-ICMP-PROPS-REF`/`TCP-IP-PROPS-REF` follow the R23-11 XSD (AUTOSAR_00052.xsd group line 50252). XSD-only legacy elements (CAN-TP-ADDRESS-REFS, DIAGNOSTIC-ADDRESS, DIAGNOSTIC-PROPS, ECU-INSTANCE-PROPSS, LIN-SLAVE, TTCAN-*, USER-DEFINED-*, TP-ADDRESS-REFS) are not modeled per Rule 0015 (absent from the PDF table; carried by no integration fixture).

## `EcuPartition`
- **PDF:** `AUTOSAR_CP_TPS_SystemTemplate.pdf`  | **page:** 201 (Table 5.7)
- **Package:** `M2::AUTOSARTemplates::SystemTemplate::SWmapping`
- **Source:** `src/armodel/models/M2/AUTOSARTemplates/SystemTemplate/SWmapping.py`

No deviations — the single Table 5.7 attribute `execInUserMode` (Boolean, 0..1, attr) is modeled as `Optional[Boolean]` + getter/setter (None-no-op, chaining); reader `readEcuPartition` (`EXEC-IN-USER-MODE` via `getChildElementOptionalBooleanValue`) and writer `writeEcuPartition` cover it; aggregated by `EcuInstance.partition` with the `PARTITIONS`/`ECU-PARTITION` wrapper per the R23-11 XSD (AUTOSAR_00052.xsd group line 50782); Base ARObject/Identifiable/MultilanguageReferrable/Referrable → Python base `Identifiable`; not VP-capable (no VARIATION-POINT anchor in group/complexType).
