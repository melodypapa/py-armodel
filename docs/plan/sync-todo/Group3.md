...
- [x] `MlFigure` (tracker input · R23-11 markdown · AUTOSAR_FO_TPS_GenericStructureTemplate · Table 9.24 · after `DocumentViewSelectable`+`Paginateable` (parents) + `LGraphic` (aggr `lGraphic`) · deps stamped: `figureCaption` Caption ✓ / `verbatim` MultiLanguageVerbatim ✓) — verified R23-11 (commit pending)
