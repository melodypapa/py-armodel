...
- [x] `TraceableTable` (dependency · **added 2026-09-11 missing-class audit** · **XSD-only** (no R23-11 markdown 9.x/E.x table, no R4.3.1 table, no PDF table — verified 2026-09-16 via grep of all three corpora + pdf_page.py) · AUTOSAR_00052.xsd `complexType name="TRACEABLE-TABLE"` line 125345 (+ group line 125328) · member type of `TopicContent.traceableTable` · **NOT in src** — class must be created when this row is synced · 16.4 decision: Derive-from-XSD → carries `# XSD verified: AUTOSAR_00052.xsd`) — verified XSD (commit <PENDING>)
