...
- [ ] `ClientServerOperationMapping` — ARObject — R23-11 markdown · Table 4.24 (CP_TPS_SoftwareComponentTemplate)
  - module: M2/AUTOSARTemplates/SWComponentTemplate/PortInterface/__init__.py
  - [x] Step 1 — Sync members & description from spec (Table 4.24, p.129; concrete Class; Base most-derived = ARObject — row confirmed; 4 attrs: argumentMapping `*` aggr of DataPrototypeMapping (already stamped, Table 4.22 — not a stub) → plural field + add/get per Rule 0001.5/1.6, and 3 × 0..1 ref → Optional[RefType] + Ref suffix, names already spec-correct; 7 class-level constr rows (1237/1240/1268/1269/1270/1875/1876) belong to this class — constr_1238 stays in ClientServerApplicationErrorMapping (constrains that class); XSD (00052 group line 17612) element order ARGUMENT-MAPPINGS / FIRST-OPERATION-REF / FIRST-TO-SECOND-DATA-TRANSFORMATION-REF / SECOND-OPERATION-REF, not VP-capable; markdown mid-word splits de-split per XSD doc wording ("ClientServer Interfaces" → "ClientServerInterfaces", "ArgumentData Prototypes" → "ArgumentDataPrototypes"); found: no class docstring, bare-RefType 0..1 annotations, untyped accessors, setters lack the None no-op guard, old 4-column checklist without `# Spec:`/release column, reader/writer drop FIRST-TO-SECOND-DATA-TRANSFORMATION-REF + ARGUMENT-MAPPINGS → Step 6 adds them; NO fake `# Spec verified:` marker found)
  - [x] Step 2 — Write model class unit test (Red) (8 tests in the mirrored test_PortInterface.py: initialization/base shape/argumentMappings add+get round-trip+chaining+None no-op/3 get-set ref round-trip+chaining+None no-op/accessor get_type_hints pins (3 Optional[RefType] pairs + add Optional[DataPrototypeMapping] + get List[DataPrototypeMapping])/spec Note + 7 constr rows verbatim incl. `__init__.__doc__ is None` + inline comment source asserts; Red confirmed — 4 None no-op tests + test_accessor_annotations (KeyError) + test_spec_note (AttributeError on None doc) failed, initialization and base shape already green)
  - [x] Step 3 — Implement model class (Green) (behavior fixes: PEP 526 `Optional[RefType]` annotations on the 3 ref fields/getter returns/setter params (0..1 per Mult.), setters gained the None no-op guard (`if value is not None`), addArgumentMapping gained the append no-op guard, setters return `"ClientServerOperationMapping"` (module has no `from __future__ import annotations` — quoted self-reference matches the sibling `ClientServerApplicationErrorMapping` pattern in this file; `List["DataPrototypeMapping"]` stays quoted — DataPrototypeMapping is defined later in the module, resolved by get_type_hints at call time); attribute blocks separated by blank lines; stale 4-column checklist removed — 6-column written at Step 7; Green = 47/48, spec-note test stays Red until Step 4's verbatim docstrings — same shape as the sibling row)
  - [x] Step 4 — Sync docstrings (wipe + rewrite) (class had no docstrings/member comments to wipe beyond the stale checklist already removed at Step 3 — wipe trivially complete; class docstring = Table 4.24 Note verbatim + 7 class-level constr rows 1237/1240/1268/1269/1270/1875/1876 (ModeRequestTypeMap/ClientIdRange precedent: attr-existence constr live in the class docstring; constr_1240's markdown bullet list joined into one sentence; constr_1268 keeps the spec's singular "two mapped ArgumentDataPrototype" quirk verbatim); 4 fields commented with per-attr Notes verbatim (markdown mid-word splits de-split per the XSD doc wording); getter/setter docstrings = Note verbatim, setters append None-no-op sentence with field name, addArgumentMapping appends the "does not append anything" sentence per the DataPrototypeMapping.addSubElementMapping sibling; Green = 48/48 in test_PortInterface.py)
  - [x] Step 5 — Write reader/writer round-trip test (Red) (7 new tests in the paired feature files: parser `TestReadClientServerInterfaceMappingOperationMappings` (test_ar_package_port_interface_mapping_set.py) — argumentMappings + 3 refs value+DEST / multiple maps in document order / absent ref → None + no ARGUMENT-MAPPINGS → [] / no OPERATION-MAPPINGS → []; writer `TestWriteClientServerInterfaceMappingOperationMappings` (same file pair) — argumentMappings + 3 refs value+DEST in XSD order / absent refs emit no elements + no ARGUMENT-MAPPINGS when list empty + no OPERATION-MAPPINGS when list empty / write→read round-trip value+DEST; Red confirmed — 4 failed (all value-carrying tests), 22 passed (pre-existing + the absent-element no-op cases))
  - [x] Step 6 — Update parser & writer (Green) (coverage COMPLETED: reader `readClientServerOperationMapping` (arxml_parser.py) added `ARGUMENT-MAPPINGS` via the shared `getDataPrototypeMappings(element, "ARGUMENT-MAPPINGS")` helper (already mutator-based: setFirstDataPrototypeRef etc.) + `addArgumentMapping`, and the missing `setFirstToSecondDataTransformationRef` via `getChildElementOptionalRefType`; writer `writeClientServerOperationMapping` (arxml_writer.py) added `setDataPrototypeMappings(child_element, "ARGUMENT-MAPPINGS", ...)` (wrapper only when non-empty, XSD minOccurs=0) + the missing FIRST-TO-SECOND-DATA-TRANSFORMATION-REF getter; element order = XSD sequenceOffset (ARGUMENT-MAPPINGS / FIRST-OPERATION-REF / FIRST-TO-SECOND-DATA-TRANSFORMATION-REF / SECOND-OPERATION-REF), matched name pairs per Rule 0013.2, no chained mutators, imports unchanged (already present); 5603 parser/writer tests green; Rule 0013 chain greps return only pre-existing `PrimType().setValue(...)` construction idioms, none on this class's lines)
  - [x] Step 7 — Update checklist comment (6-column format with release column, 9 rows in accessor source order (Rule 0001.11 list-shape mutator-first fixed at this step: addArgumentMapping moved before getArgumentMappings, matching the DataPrototypeMapping sibling); reader [x] on add/set rows verified against `readClientServerOperationMapping` (arxml_parser.py, ARGUMENT-MAPPINGS via getDataPrototypeMappings + addArgumentMapping, 3 refs via setters), writer [x] on get rows verified against `writeClientServerOperationMapping` (arxml_writer.py, ARGUMENT-MAPPINGS via setDataPrototypeMappings + 3 refs via getters); set-based checklist-vs-methods script passes; NO `# Spec verified:` — deferred to batch confirmation)
  - [x] Step 8 — Deviations (fixed+recorded in step notes: bare-RefType 0..1 fields + untyped accessors → PEP 526 `Optional[RefType]` typed accessors; setters lacked the None no-op guard → guard added + addArgumentMapping append no-op; reader/writer dropped FIRST-TO-SECOND-DATA-TRANSFORMATION-REF + ARGUMENT-MAPPINGS → coverage completed at Step 6; old 4-column checklist → 6-column with `# Spec:` line + release column; accessor pair order get-before-add → mutator-first per Rule 0001.11; NO fake `# Spec verified:`/`# XSD verified:` marker was present in the class body (intake was docstring-less and marker-less) — nothing to remove; deviation trackers docs/examples/method_deviation_by_class.md + v2 have NO entries for this class — nothing stale; no open deviations, no placeholders — RefType is the spec type for all three `ref` rows, argumentMapping's DataPrototypeMapping member type is already stamped (Table 4.22), ref targets ClientServerOperation/DataTransformation exist)
  - [ ] Step 9 — Verify (9a) + confirm (9b) — 9a passed 2026-09-26: 15 new tests (8 model + 4 parser + 3 writer); set-based checklist-vs-methods script green; full suite 12153/0 (baseline 12138 +15); `npm run lint` (flake8 + ruff) + `npm run black` + `black-check` green; 9b deferred to batch confirmation (user instruction)
...
- [ ] `ClientServerInterfaceMapping` — PortInterfaceMapping — source TBC (locate table at Step 1)
  ...
  - [x] Step 1 — Table 4.23 (AUTOSAR_CP_TPS_SoftwareComponentTemplate.md L3772, p.128 via pdf_page.py; R4.3.1 reproduction Table 4.27 p.130 same rows), concrete Class; Package M2::AUTOSARTemplates::SWComponentTemplate::PortInterface; Note "Defines the mapping of ClientServerOperations in context of two different ClientServerInterfaces." verbatim; Base row ARObject, AtpBlueprint, AtpBlueprintable, Identifiable, MultilanguageReferrable, PortInterfaceMapping, Referrable → most-derived provided base PortInterfaceMapping (Table 4.20, stamped — remaining entries are its ancestors); Aggregated by PortInterfaceMappingSet.portInterfaceMapping; 2 attribute rows both `*` aggr: errorMapping (ClientServerApplicationErrorMapping — Note "Map two different ApplicationErrors defined in the context of two different ClientServerInterfaces.") then operationMapping (ClientServerOperationMapping — Note "Mapping of two ClientServerOperations in two different ClientServerInterfaces Stereotypes: atpSplitable Tags: atp.Splitkey=operationMapping"); XSD 00052 complexType L17284 composes AR-OBJECT+REFERRABLE+MULTILANGUAGE-REFERRABLE+IDENTIFIABLE+ATP-BLUEPRINT+ATP-BLUEPRINTABLE+PORT-INTERFACE-MAPPING + own group L17251 (ERROR-MAPPINGS wrapper before OPERATION-MAPPINGS = table displayed order); no constr rows; no class-level fake marker found (intake was marker-less with a stale 4-column checklist)
  - [x] Step 2 — new TestClientServerInterfaceMappingSpecContract (6 tests in test_PortInterface.py: initialization defaults, base-shape pin (PortInterfaceMapping/Referrable/MultilanguageReferrable/Identifiable/AtpBlueprintable/ARObject), errorMappings add/get round-trip+chaining+None no-op, operationMappings same, get_type_hints accessor pins (List returns, Optional value, chained self returns), spec Note verbatim incl. __init__.__doc__ is None + inline comment source asserts); Red confirmed — test_accessor_annotations KeyError + test_spec_note failed, 4 behavior tests already green (sibling-row shape)
  - [x] Step 3 — class block replacement (Steps 3+4 combined per module convention): no behavior change (two typed list fields + add/get accessors already present and correct); fixes = typed accessor annotations (List[...] returns, Optional[...] add values, quoted self returns per module convention — no future-annotations here) + None no-op sentences; Green 54/54 in test_PortInterface.py after Step 4
  - [x] Step 4 — whole class block replaced: class docstring = Table 4.23 Note verbatim; stale 4-column checklist removed (6-column written at Step 7); both fields commented with per-attr Notes verbatim (incl. the atpSplitable Tags cell); getter/adder docstrings = Note verbatim, adders append the None-no-op sentence; one transcription drift caught and fixed before commit (addOperationMapping docstring had dropped "Stereotypes: atpSplitable")
  - [x] Step 5 — class-level orchestrator coverage added (the ERROR-MAPPINGS and OPERATION-MAPPINGS sub-wrappers were already test-pinned by the two sibling rows): parser TestReadClientServerInterfaceMapping (2: full PORT-INTERFACE-MAPPING-SET dispatch with SHORT-NAME + both wrappers value/DEST asserts; empty mapping → wrappers absent, lists empty) + writer TestWriteClientServerInterfaceMapping (3: field values in XSD order with ERROR-MAPPINGS < OPERATION-MAPPINGS index assert, empty mapping omits wrappers, write→read round-trip with xmlns injection); reader side NOT Red (coverage already existed and behaved spec-correctly — harness-only outcome like the ModeRequestTypeMap row); writer Red: NameError missing test import (harness) + IndexError — REAL trap re-confirmed: direct ClientServerInterfaceMapping(parent, name) construction bypasses addElement so PortInterfaceMappingSet.getPortInterfaceMappings() returns [] — switched to the createClientServerInterfaceMapping factory (PostBuildVariantCriterionValue precedent)
  - [x] Step 6 — orchestrator verified PRESENT and spec-correct, N/A: readClientServerInterfaceMapping (arxml_parser.py) = readIdentifiable + ERROR-MAPPINGS dispatch (isinstance CLIENT-SERVER-APPLICATION-ERROR-MAPPING → readClientServerApplicationErrorMapping, else notImplemented) + OPERATION-MAPPINGS dispatch (→ readClientServerOperationMapping complete after dbabba48c); writer mirrored via getters; wrapper order = XSD sequenceOffset; matched name pairs per Rule 0013.2, no chained mutators; the notImplemented else-branches remain for genuinely unknown sub-tags (defensive, spec has no other subtypes)
  - [x] Step 7 — 6-column checklist with release column written in the class block: # Spec: R23-11/AUTOSAR_CP_TPS_SoftwareComponentTemplate.pdf, Table 4.23, p.128 (R23-11); 5 rows in accessor source order (__init__ [—/—], getErrorMappings writer [x], addErrorMapping reader [x], getOperationMappings writer [x], addOperationMapping reader [x]) verified against the readClientServerInterfaceMapping/writeClientServerInterfaceMapping call sites; base-composition + own-group-order notes in the header; NO # Spec verified: — deferred to batch confirmation
  - [x] Step 8 — Deviations [none open: field-to-spec both directions exact (exactly the two `*` aggr rows → two typed lists, Rule 0004); no naming/type deviation; no fake intake marker existed; the intake untyped accessors + missing docstrings were to-fix items fixed in-pass (Rule 0014: fixed ⇒ no rows); tracker v1/v2 have no entries for this class; referenced-missing: NONE — both member types exist and are synced (ClientServerApplicationErrorMapping Table 4.25, ClientServerOperationMapping Table 4.24 committed dbabba48c)]
  - [ ] Step 9 — Verify (9a) + confirm (9b) — 9a passed 2026-09-26: target files 70/70 (test_PortInterface.py 54 + paired mapping-set parser/writer suites), full suite `uv run python scripts/run_tests.py --no-coverage` 12164 passed / 0 failed (baseline 12153 + 11 net-new: 6 model + 2 parser + 3 writer), `npm run lint` clean, black no-op + `black-check` clean (1258 files); 9b deferred to batch confirmation (user instruction)
