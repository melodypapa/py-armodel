...
### 18.5 Stop and ask on blockers (executing-plans discipline)

Each step is executed like a plan task: follow the step procedure exactly,
run its verification, check it off (18.2) — and **stop and ask the user the
moment a step cannot proceed honestly by the rules**. A blocker is any of:

- **Conflicted/ambiguous spec source.** Two divergent tables for the class —
  e.g. a stale older-release markdown copy in `autosar/markdown/` whose table
  id collides with the pinned release's table — or table rows that cannot be
  reconciled with the pinned release. Even when the release pin makes one
  table "obviously" correct, the collision is the **user's call**: present
  both sources and a proposed resolution, get an answer, then proceed.
  Surfacing the conflict only at 9b is too late — 9b certifies a finished
  class; the conflict determines what gets built.
- **Unclear instruction.** A step's procedure or a rule does not cover the
  situation at hand. Ask; do not improvise a third option (mirrors 16.4's
  no-invented-options rule).
- **Verification fails repeatedly.** The same check (typically the 9a
  round-trip) still fails after **two** genuine fix attempts. Stop iterating;
  present the observed evidence (emitted XML, XSD diff, isolated reader test)
  and the remaining within-rules options. Never relax a check to make it pass;
  never stamp over a dropped element.

An acceptable stop carries **evidence and a proposal** — not an empty
"I'm stuck".

**Not blockers** (do not stop for these): a referenced member type found
missing at Step 3 (collect it and report at Step 8 — Rule 0001.10, do not
block); steps that are N/A for the class shape (Workflow adaptations);
anything the rules fully determine on their own.

Blocker stops are **exception gates**, not routine confirmations — they fire
only when a step cannot proceed, and they never substitute for or weaken the
9b confirmation gate (18.3).
