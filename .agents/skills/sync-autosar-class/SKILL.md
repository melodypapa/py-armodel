...
- **Reordering class members/checklist rows to XSD `sequenceOffset` because the markdown
  table's row order looks alphabetical** — the markdown/PDF displayed order (even a
  page-split, alphabetical-looking render) is still the class member order (*Rule
  0001.11*); `sequenceOffset` governs **only** the reader/writer XML element order. These
  are two independent orders — fixing one by copying the other is the violation.
